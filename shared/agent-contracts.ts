...
// ==========================================
// 5. Agent Permission Tiers, Site Policy & Audit Ledger
// ==========================================

/** Least-privilege tiers, ordered read < navigate < interact < sensitive. */
export type AgentPermissionTier = 'read' | 'navigate' | 'interact' | 'sensitive';

export interface SiteAgentPolicy {
  domain: string;
  maxTier: AgentPermissionTier;
...
export interface AgentPolicyStore {
  version: 1;
  defaultMaxTier: AgentPermissionTier;
  sites: Record<string, SiteAgentPolicy>;
}

export type AgentAuditVerdict = 'auto-approved' | 'approved' | 'denied' | 'blocked';
...
export interface AgentPolicyVerdict {
  allowed: boolean;
