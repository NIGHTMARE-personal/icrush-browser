...
exports.hasSensitiveFields = hasSensitiveFields;
exports.generateSensitiveFieldDetectionJS = generateSensitiveFieldDetectionJS;
exports.generateHighlightJS = generateHighlightJS;
exports.generateTooltipJS = generateTooltipJS;
exports.generateClickRippleJS = generateClickRippleJS;
exports.generateActionJS = generateActionJS;
exports.parseAgentAction = parseAgentAction;
/**
 ...
 */
function isSensitiveField(selector, _value) {
 ...
 */
function hasSensitiveFields(fields) {
 ...
 */
function generateSensitiveFieldDetectionJS() {
    return `
    (function() {
 ...
 */
function generateClickRippleJS(x, y, color = '#6366f1') {
    return `
    (function() {
 ...
 */
function generateClickJS(selector) {
    return `
    (function() {
      ${SHADOW_DOM_HELPERS}
 ...
 */
function generateTypeJS(selector, text) {
 ...
 */
function generateFillFormJS(fields) {
    return `
    (function() {
      ${SHADOW_DOM_HELPERS}
      const results = [];
 ...
 */
function generateScrollJS(direction, amount) {
    const px = amount || 500;
    const yMap = { up: -px, down: px };
    const xMap = { left: -px, right: px };
 ...
 */
function generateSelectJS(selector, value) {
    return `
    (function() {
      ${SHADOW_DOM_HELPERS}
      const el = queryDeep(${JSON.stringify(selector)});
 ...
 */
function generatePressKeyJS(keys) {
    return `
    (function() {
      const keyMap = {
...
function generateActionJS(action) {
    switch (action.type) {
        case 'click':
            if (!action.selector)
                return 'JSON.stringify({ success: false, error: "No selector provided for click" })';
        ...
        case 'select':
            if (!action.selector)
                return 'JSON.stringify({ success: false, error: "No selector provided for select" })';
            return generateSelectJS(action.selector, action.value || '');
        case 'hover':
            if (!action.selector)
                return 'JSON.stringify({ success: false, error: "No selector provided for hover" })';
            return `
        (function() {
          ${SHADOW_DOM_HELPERS}
      ...
      `;
        case 'press_key':
            if (!action.keys)
                return 'JSON.stringify({ success: false, error: "No keys provided" })';
...
function parseAgentAction(rawAction) {
    const actionType = (rawAction.action_type || rawAction.type || rawAction.action || 'extract');
    const validTypes = ['click', 'type', 'scroll', 'extract', 'fill_form', 'select', 'hover', 'wait', 'press_key'];
    const type = validTypes.includes(actionType) ? actionType : 'extract';
    return {
        type,
        selector: (rawAction.selector || rawAction.element || rawAction.target),
        text: (rawAction.text || rawAction.content || rawAction.value),
        value: (rawAction.value || rawAction.option),
        direction: (rawAction.direction || 'down'),
        amount: (rawAction.amount || rawAction.pixels || 500),
        keys: (rawAction.keys || rawAction.key),
        formFields: rawAction.formFields,
    };
}
