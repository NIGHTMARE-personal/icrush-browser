...
class ToolRegistry {
    constructor(userDataPath) {
        ...
        Object.defineProperty(this, "rateLimitCounters", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: new Map()
        });
        Object.defineProperty(this, "userDataPath", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: void 0
    ...
    getToolsByTier(tier) {
        const tierRank = { read: 0, navigate: 1, interact: 2, sensitive: 3 };
    ...
    async execute(request, context) {
        ...
        try {
            const output = await this.executeWithTimeout(() => executor(request.params, ctx), tool.timeoutMs);
            const result = {
                callId: request.callId,
                toolId: request.toolId,
                success: true,
    ...
    registerBuiltInTools() {
        ...
        this.executors.set('read_file', async (params, ctx) => {
            ...
            return fs.readFileSync(filePath, String(params.encoding));
        });
        // Write file
        ...
        this.executors.set('screenshot', async (params, ctx) => {
            if (!ctx.webview)
                throw new Error('No webview available for screenshot');
    ...
    async fetchPage(url, format, headers, signal) {
        const res = await fetch(url, {
            headers: {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                ...
                if (result && typeof result === 'object' && typeof result.then === 'function') {
                    result.then(r => { clearTimeout(timer); resolve(r); }).catch(e => { clearTimeout(timer); reject(e); });
                }
                else {
                    clearTimeout(timer);
                    resolve(result);
