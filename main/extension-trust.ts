...
export type ExtensionRiskLevel = 'low' | 'elevated' | 'high' | 'blocked';

export interface ExtensionTrustReport {
  name: string;
  version: string;
 ...
 */
export function assessManifest(manifest: unknown, extensionId?: string): ExtensionTrustReport {
  const blank: ExtensionTrustReport = {
    name: 'unknown',
    version: 'unknown',
    manifestVersion: 0,
    permissions: [],
    ...
    for (const cs of manifest.content_scripts) {
      if (isRecord(cs)) {
        for (const m of asStringArray(cs.matches)) {
          if (!hostPermissions.includes(m)) hostPermissions.push(m);
        }
      }
    }
  }

  const report: ExtensionTrustReport = {
    name,
    version,
    manifestVersion,
...
export function parseCrxZipRange(data: Uint8Array): { start: number; format: CrxFormat } {
  ...
  if (isCrx3) {
    if (data.length < 12) throw new Error('Truncated CRX3 header');
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    const headerSize = view.getUint32(8, true);
    const start = 12 + headerSize;
    if (start >= data.length) throw new Error('CRX3 header claims more bytes than the file holds');
    return { start, format: 'crx3' };
  }
  if (isCrx2) {
    if (data.length < 10) throw new Error('Truncated CRX2 header');
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    const headerSize = view.getUint16(8, false);
    const start = 10 + headerSize;
    if (start >= data.length) throw new Error('CRX2 header claims more bytes than the file holds');
  ...
  if (isHtml) {
    throw new Error(
      'Server returned an HTML page instead of an extension file. The URL may be blocked or incorrect.'
    );
  }
...
export function matchesBlocklist(extensionId: string | undefined, blocklist: Set<string>): boolean {
 ...
 */
const KNOWN_MALICIOUS_IDS: string[] = [
  // Crypto miners
 ...
 */
export function getSeedBlocklist(): Set<string> {
 ...
 */
export function mergeBlocklists(existing: Set<string>, additional?: Set<string>): Set<string> {
