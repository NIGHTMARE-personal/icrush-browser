...
class MCPBridge {
    constructor() {
        ...
        this.configPath = path_1.default.join(electron_1.app.getPath('userData'), 'mcp-servers.json');
    }
    ...
    init() {
        ...
        if (fs_1.default.existsSync(this.configPath)) {
            try {
                ...
                const customServers = JSON.parse(raw);
    ...
    saveCustomConfigs() {
        try {
            ...
            fs_1.default.writeFileSync(this.configPath, JSON.stringify(customList, null, 2), 'utf8');
        }
        catch (err) {
    ...
    listTools() {
        this.init();
        const enabledServers = new Set(Array.from(this.servers.values())
            .filter(s => s.enabled)
            .map(s => s.id));
    ...
    getAuthorizationUrl(server, state) {
    ...
    async handleOAuthCallback(code, serverId) {
        const server = this.servers.get(serverId);
        if (!server)
            throw new Error('Server not found');
        try {
    ...
    async exchangeCodeForToken(code, server) {
    ...
    async refreshAccessToken(serverId) {
        const server = this.servers.get(serverId);
        ...
        try {
            ...
            if (data.refresh_token)
                server.refreshToken = data.refresh_token;
            server.authenticated = true;
            this.saveCustomConfigs();
            return true;
        }
...
exports.mcpBridge = new MCPBridge();
